...
#            merra_lut = load_cams(config['cams'])
        ...
        if len(good[0]) == 0:
            ...
            filtre = (taup550 > config['taot'])
            data['ac_process_flag'].data[good] |= filtre.astype('u1')
    ...
    #        Juo3_ext = np.zeros((NB, GSIZEXT), dtype='float32') + np.NaN
    #        Juh2o_ext = np.zeros((NB, GSIZEXT), dtype='float32') + np.NaN
